...
export default function DetailedDocument(params){
    ...
    if(!doc || (doc.permit == 'private' && !logData.isLogged)) {setLocation('/');}
    useEffect(() => {
    ...
    const Download = evt => {
        evt.preventDefault()
        axios.post(API_URL+'download', doc).then(response => console.log(response))
    }
    const editFile = e => {
        e.preventDefault();
    ...
    return <>
            ...
            <span className='Fa-edit' onClick={() => window.history.go(-1)}><IoReturnUpBack></IoReturnUpBack></span>
                <h2 className='Titulo'>
                    {doc.title ? doc.title : null }
                </h2>
                <span className='Date'>{doc.Date ? doc.Date : null }</span>
                {edit ? <span className='Fa-edit' onClick={editFile} ><FaEdit></FaEdit></span> : null}
            </div>
            <div className='Body-noticia'>
                <img src={doc.img ? doc.img : null } onError = {e => e.target.style.display = 'none'} alt=''/>
                <p className='Informacion'>{doc.text ? doc.text : 'No such File'}</p>
            </div>
            <div className='Footer-noticia'>
                {doc.file ? 
            <Button className='Download-button' variant="success" onClick={Download}>Descarga</Button> : null}
