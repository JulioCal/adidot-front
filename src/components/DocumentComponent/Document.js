...
export default function Document({id, title, img, text, owner}){
    ...
    return <div className='Noticia' onClick={documentDetail}>
            <div className='Header-noticia'>
                <h3 className='Titulo'>
                    {title}
                </h3>
                <span className='Date'>{current.getDate() + '/'+ (current.getMonth()+1)}</span>
            </div>
            <div className='Body-noticia'>
                <img src={img} onError = {e => e.target.style.display = 'none'} alt=''/>
